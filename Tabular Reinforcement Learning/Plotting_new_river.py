...
#plt.fill_between(np.arange(30000), Q_learning_final - 1.96 * Q_learning_std, Q_learning_final + 1.96 * Q_learning_std, color= 'purple', alpha=0.2)


plt.plot(double_Q_learning_final, color='blue', label='Double Q-learning')
#plt.fill_between(np.arange(30000), double_Q_learning_final - 1.96 * double_Q_learning_std, double_Q_learning_final + 1.96 * double_Q_learning_std, color= 'blue', alpha=0.2)

plt.plot(expected_sarsa_final, color='red', label='Expected Sarsa')
#plt.fill_between(np.arange(30000), expected_sarsa_final - 1.96 * expected_sarsa_std, expected_sarsa_final + 1.96 * expected_sarsa_std, color= 'red', alpha=0.2)

plt.plot(sarsa_final, color='green', label='Sarsa')
#plt.fill_between(np.arange(30000), sarsa_final - 1.96 * sarsa_std, sarsa_final + 1.96 * sarsa_std, color= 'green', alpha=0.2)

plt.legend()

plt.grid()

plt.savefig('Comparison_Smoothed_River')
